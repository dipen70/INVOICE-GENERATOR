...
def _invoice_details(parent):
    ...
    state["details"]["issue_date"] = _field_row(card, "Issue Date:", 0, col=2, default=today.strftime("%Y-%m-%d"))
    state["details"]["due_date"]   = _field_row(card, "Due Date:",   0, col=4, default=due.strftime("%Y-%m-%d"))
    ...
    cb_currency = ttk.Combobox(card, width=16, state="readonly",
        values=["USD ($)", "EUR (€)", "GBP (£)", "INR (₹)", "JPY (¥)", "CAD (CA$)", "AUD (A$)"])
    cb_currency.set("USD ($)")
